...
def train(epochs,train_loader,val_loader,test_loader):
    ...
    for epoch in range(epochs):
        ...
        # 打印当前epoch的结果
        print(f'Epoch {epoch + 1}/{epochs}, Train_Loss: {epoch_loss/len(train_loader):.4f}, Val_Loss: {val_loss:.4f}, Train_Acc: {train_acc:.4f}, Val_Acc: {val_acc:.4f}')
        # 更具val的acc调整学习率2
...
if __name__ == "__main__":
    # 加载数据loading data
    ## 加载图片数据集
    dataset=ImageFolder(root='image/train',transform=transform)
    ## 按照比例分层划分数据集为训练集和验证集
    train_dataset,val_dataset=split_dataset(dataset,0.85)
    val_dataset.dataset.transform = val_transform
    ## 加载测试集的10张照片
    test_dataset=TestDataset(root='image/test',classes=dataset.classes,transform=val_transform)
